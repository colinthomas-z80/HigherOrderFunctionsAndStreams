...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testFilter() {
        ...
        List<String> actual = input.filter(i -> i.startsWith("b")).collect(Collectors.toList());
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(s -> String.valueOf(s.charAt(0))).distinct().collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(s -> Integer.valueOf(s.charAt(1)) % 2 == 0).collect(Collectors.joining());
    ...
    public void testCountByDate() {
        ...
        long actual = input.filter(s -> s.timestamp.isBefore(instant)).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountWhitelisted() {
        ...
        long actual = input.filter(s -> whitelist.contains(s.user) == true).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(s -> s.user.toLowerCase().contains("bot") == false).count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(s -> s.timestamp
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = input.map(s -> s.timestamp
                .atZone(ZoneId.systemDefault())
                .getMonth())
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = input.map(s -> s.timestamp
                .atZone(ZoneId.systemDefault())
                .getMonth())
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
                .entrySet()
